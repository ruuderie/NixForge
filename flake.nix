{
  ...
  outputs = { self, nixpkgs, disko, sops-nix, darwin, colmena, ... }@inputs: let
    # Shared modules list for reuse in configurations and colmena
    managerModules = [
      disko.nixosModules.disko  # Include disko module for disk config
      sops-nix.nixosModules.sops  # Include sops-nix module for secrets

      # Main configuration module (anonymous)
      ({ config, pkgs, ... }: {
